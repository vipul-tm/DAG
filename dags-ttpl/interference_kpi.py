...
from subdags.interference_kpi_subdag import process_interference_kpi
...
PARENT_DAG_NAME = "INTERFERENCE_KPI"
#3-59/5 * * * *
# 3,8,13,18,23,28,33,38,43,48,53,58 * * * *
interference_dag=DAG(dag_id=PARENT_DAG_NAME, default_args=default_args, schedule_interval='*/15 * * * *')
redis_hook_10 = RedisHook(redis_conn_id="redis_hook_10")
...
def init_kpi():
    ...
    redis_hook_10.flushall("*")
    logging.info("Flushed all in redis_hook_10 connection")
...
def update_age(**kwargs):
    try:
        ...
        aggregated_data_vals = list(redis_hook_10.get_keys("aggregated_interference_*_%s_%s"%(bs_or_ss,device_type)))
...
for technology in technologies:
    ...
    ul_subdag_task=SubDagOperator(subdag=process_interference_kpi(PARENT_DAG_NAME, CHILD_DAG_NAME, datetime(2017, 2, 24),interference_dag.schedule_interval,Q_PUBLIC,
